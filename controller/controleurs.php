...
function new_Chapter($message = null) {
	require("backView/ajouterunchapitre.php");
}
function update_Chapter($chapter_title, $chapter_text, $img_url, $chapter_id){
	$billetsManagerInstance = new billetsManager();
    $update_Chapter = $billetsManagerInstance->update_Chapter($chapter_title, $chapter_text, $img_url, $chapter_id);
}
...
function message_admin($msg_query, $msg_id){
	$contactManagerInstance = new contactManager();
	if ( $msg_query === "reply" ) {
		require("backView/");
	} 
	if ( $msg_query === "delete" ) {
		$message = $contactManagerInstance->msg_query($msg_id);
		message();
	} 
}
