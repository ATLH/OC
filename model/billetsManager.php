...
class BilletsManager {
	...
	public function update_Chapter($chapter_title, $chapter_text, $img_url, $chapter_id){
		$bdd = $this->bddConnexion();
		$bdd_update_chapter = $bdd->prepare("UPDATE chapitre SET chapter_title = ?, chapter_text = ?, img_url = ?, chapter_date = CURDATE() WHERE ID = ?");
		$bdd_update_chapter->execute(array($chapter_title, $chapter_text, $img_url, $chapter_id));
	}
